...
public class LinemanPanel extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			pst = conn.prepareStatement("SELECT complaint.comp_id," + "fk_login_id," + " consumer_personal.consumer_id,"
					+ " complaint," + " comp_date," + " comp_status," + "consumer_name," + " consumer_phone,"
					+ "consumer_loc," + "consumer_post\r\n" + "FROM lineman_duty INNER JOIN complaint\r\n"
			...
			rs = pst.executeQuery(
					"SELECT complaint.comp_id,fk_login_id, consumer_personal.consumer_id, complaint, comp_date, comp_status,consumer_name, consumer_phone,consumer_loc,consumer_post\r\n"
							+ "FROM lineman_duty INNER JOIN complaint\r\n"
							+ "ON complaint.comp_id=lineman_duty.comp_id\r\n" + "AND fk_login_id=" + login_id + "\r\n"
							+ "INNER JOIN consumer_personal\r\n"
							+ "ON consumer_personal.consumer_id=complaint.consumer_id");
			...
			out.println("<div class=\"complaintTable\">\r\n" + "<table class=\"comp\">" + "<tr><th>Consumer ID</th>"
					+ "<th>Complaint ID</th>" + "<th>Date</th>" + "<th>Complaint</th>" + "<th>Status</th>"
					+ "<th>Name</th>" + "<th>Phone</th>" + "<th>location</th>" + "<th>Post</th>" + "</tr>");
			while (rs.next()) {
				out.println(" <tr>\r\n" + "                <td>" + rs.getInt(3) + "</td>\r\n" + "                <td>'"
